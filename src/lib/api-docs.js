...
import {TRIBE_API_ENDPOINTS} from './tribe-api-docs.js';
const jsonBody = (example) => ({ contentType: "application/json", example });

export const API_ENDPOINTS = [
  ...TRIBE_API_ENDPOINTS,
  {tag:'Проекты',method:'POST',path:'/api/projects/{projectId}/access/owner',title:'Передать владение проектом',description:'Только владелец проекта или системный администратор. user_id — активный сотрудник трайба. revision обязательна; владелец защищён от удаления и понижения роли.',scope:'projects:write',permission:'project.access.manage',body:jsonBody({user_id:12,revision:1})},
  {tag:'Проекты',method:'PATCH',path:'/api/projects/{projectId}/access/mode',title:'Закрыть существующий проект',description:'Только владелец проекта или системный администратор. Переводит прежнюю схему компании в доступ только по назначениям.',scope:'projects:write',permission:'project.access.manage',body:jsonBody({access_mode:'members',revision:1})},
  {tag:'ИИ-агенты',method:'GET',path:'/api/work/agents/approvals',title:'Мои согласования ИИ',description:'Ожидающие шаги и предложенные действия. Проверяет согласующего, срок, актуальные права проекта и источников. Для решения используются существующие /agents/runs/{runId}/gate и /review. Возвращает items; рассматриваются до 500 ожидающих запусков компании.',scope:'agents:read',permission:'agent.view + agent.approve'},
  ...WORK_API_ENDPOINTS,
  ...
  { tag: "Проекты", method: "GET", path: "/api/projects/{projectId}/access", title: "Доступ пользователей и групп", description: "Возвращает владельца, tribe_id, access_mode, revision, личные и групповые назначения, доступных сотрудников трайба и допустимые роли.", scope: "projects:read", permission: "project.access.manage" },
  { tag: "Проекты", method: "POST", path: "/api/projects/{projectId}/access", title: "Назначить проектную роль", description: "principal_type: user или group; project_role: manager, member, viewer. revision защищает от устаревшей формы (409). Для личного назначения нужно членство в трайбе. Нельзя выдать отсутствующие у себя права или понизить владельца/последнего руководителя.", scope: "projects:write", permission: "project.access.manage", body: jsonBody({ principal_type: "group", principal_id: 3, project_role: "member", revision: 1 }) },
  ...
  ...["user", "group"].map((type) => ({ tag: "Проекты", method: "DELETE", path: `/api/projects/{projectId}/access/${type}/{principalId}`, title: `Удалить назначение ${type === "user" ? "пользователя" : "группы"}`, description: "Убирает один источник доступа; другие роли и группы могут сохранить права. revision передаётся в query; владельца удалить нельзя.", scope: "projects:write", permission: "project.access.manage" })),
  ...
  { tag: "Проекты", method: "POST", path: "/api/projects", title: "Создать проект", description: "Создаёт закрытый проект из пустой конфигурации или шаблона. tribe_id выбирает трайб с правом создания; автор становится владельцем. Без tribe_id старый контракт доступен только системному администратору.", scope: "projects:write", permission: "can_create_projects в выбранном трайбе", body: jsonBody({ tribe_id: 1, name: "Новый продукт", key_code: "PROD", description: "Развитие продукта" }) },
...
// Собирает актуальное описание API и способов доступа для текущей версии приложения.
