...
## Проверки трайбов и согласований в 0.23

`npm run test:tribes` проверяет реальные права, группы, владельцев и согласования
на выбранном одноразовом стенде. Команда входит в `test:integration` и создаёт
учётные записи и ожидающие решения для браузерных тестов. Выполняйте её до
`test:browser`; рабочий `.env` эти проверки не используют.
